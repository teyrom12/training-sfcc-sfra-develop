...
server.get('Show', cache.applyPromotionSensitiveCache, consentTracking.consent, function (req, res, next) {
    ...
    // Handle 404 if product is not online or invalid product type
    if (!showProductPageHelperResult.product.online && productType !== 'set' && productType !== 'bundle') {
        ...
        if ((pageLookupResult.page && pageLookupResult.page.hasVisibilityRules()) || pageLookupResult.invisiblePage) {
            res.cachePeriod = 0; // Do not cache if visibility rules are present
        }

        if (pageLookupResult.page) {
            res.page(pageLookupResult.page.ID, {}, pageLookupResult.aspectAttributes);
        } else {
            // Fetch suggested products based on the category of the current product
            ...
            if (product && product.primaryCategory) {
                ...
                searchModel.setCategoryID(categoryID); // Search for products in the same category
                searchModel.search(); // Execute the search

                // Fetch up to 4 suggested products
                suggestedProducts = searchModel.getProductSearchHits().asList().slice(0, 4);
            }

            // Render the product detail page and pass the suggested products
            res.render(showProductPageHelperResult.template, {
                ...
                suggestedProducts: suggestedProducts // Pass the suggested products to the ISML template
            });
        }
    }
    next();
